...
const fontSizes = () => { 
    let content = '';
    for(const size in defaultStyles.fontSize) {
        content += `.font-${size} { font-size: ${ defaultStyles.fontSize[size][0] }; line-height: ${ defaultStyles.fontSize[size][1].lineHeight } }`;
    }
    return content;
}
...
const GlobalStyle = createGlobalStyle`
    * {
    ...
    ::-webkit-scrollbar-thumb { background: ${({theme}) => theme.bg.secondary}; border: 4px  solid ${({theme}) => theme.bg.default}; border-radius: .75rem }
    .remark-highlight > pre::-webkit-scrollbar-thumb:horizontal { border-color: ${({theme}) => theme.border.light} }
    ...
    .bg-light { background-color: ${({theme}) => theme.bg.light} }
    ...
    .text-gr-primary{ color: transparent;background-clip: text;-webkit-background-clip:text;background-image: linear-gradient(90deg, ${({theme}) => theme.bg.primary}, ${({theme}) => theme.colors.green[200]}) }
    ...
    .post-layout, .plain-layout, .df-layout { display: inline-block; width: 100% }
